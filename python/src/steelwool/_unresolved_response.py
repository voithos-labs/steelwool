...
from ._enums import ContentType, MessageRole, StopReason
...
class UnresolvedResponse(BaseModel):
    ...
    async def resolve_tool_calls(self, tool_resolver: ToolResolver) -> ContextBuilder:
    ...
    async def resolve_tool_calls_recursively(
        ...
        # Base case
        if tool_reprompt_death == 0 or max_tokens <= 0:
        ...
        if self.prompt_response.stop_reason in (
            StopReason.STOP,
            StopReason.LENGTH,
        ...
        # keep reprompting until prompt death == 0, max_tokens <= 0, or there is no more tools needed to be called!
            ...
            for tc in self.prompt_response.tool_calls:
                ...
                return await unresolved_response.resolve_tool_calls_recursively(
            ...
            raise Exception(
                "Unexpected StopReason.NULL encountered during tool call resolution"
            )

    async def resolve_with(
        self, resolver: Callable[["UnresolvedResponse"], Awaitable[ContextBuilder]]
    ) -> ContextBuilder:
        """
    ...
    def resolve_with_sync(
        self, resolver: Callable[["UnresolvedResponse"], ContextBuilder]
    ) -> ContextBuilder:
        """
